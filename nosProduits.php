...
require_once ('include/sideBar.php');
	...
	if(isset($_GET['show'])){
		$product = $_GET['show'];
		$select = $db -> prepare("SELECT * FROM produits WHERE libelle_produit='$product'");
		$select -> 	execute();
	...
	if(isset($_GET['category'])){
		...
		$select = $db -> query("SELECT * FROM produits WHERE id_type_produit='$category'");
		$select -> 	execute();
		...
		while($s=$select->fetch(PDO::FETCH_OBJ)){
			...
			?>
					...
					< ?php if($s->stock!=0){?><a href="panier.php?action=ajout&amp;l=< ?php echo $s->title;?>&amp;q=1&amp;p=< ?php echo $s->price;?>"><button>Ajouter au panier</button></a>< ?php }else{echo'<h5 style="color:red;">Stock �puis� !</h5>';}?>
					<br/><br/>
					-->
				</div>
